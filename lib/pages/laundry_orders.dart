  ...
  Widget build(BuildContext context) {
    return ListView.builder(
        ...
        itemBuilder: (BuildContext context,i) {
          ...
          if(statusId == 1){ orderStatus = "Recibido";}
          else if(statusId == 2){orderStatus = "En camino";}
          else{orderStatus = "Entregado";}
          ...
          return Container(
                  ...
                  child: Container(
                    ...
                    child: InkWell(
                      child: Row(
                        children: [
                          Icon(Icons.store,color: Color.fromRGBO(121, 47, 218, 1),size: 50),
                      ...
                      onTap: () {
                        Navigator.push(context,
                            MaterialPageRoute(
                                builder: (context) => LaundryOrderDetails(data[i]['id'],data[i]['userId'],orderStatus)
                            )
                        );
                      },
                    ),
...
class _LaundryOrderDetailsState extends State<LaundryOrderDetails> {
  ...
  Future<String> updateOrderStatus(String orderStatus, int userId, String deliveryAddress, double orderAmount, String date, String deliveryDate) async {
    int orderStatusId = 0;

    if(orderStatus == "Recibido"){ orderStatusId = 1;}
    else if(orderStatus == "En camino"){orderStatusId = 2;}
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to update order.' + userId.toString() +"//"+ orderStatusId.toString()+"//" + deliveryAddress +"//"+ orderAmount.toString()+"//" + date+"//" + deliveryDate);
    }
  }
          ...
          child: Column(
            children: <Widget>[
              Padding(
                ...
                child: Text(
                  userData[userId]["firstName"] + " " + userData[userId]["lastName"],
                  style: TextStyle(fontSize: 24,color: Color.fromRGBO(121, 47, 218, 1)),
                ),
              ),
              Container(
                child: Row(
                  children: <Widget>[
                    Padding(
                      ...
                      child: Text(
                        "Fecha de recepción: ", style: TextStyle(fontSize: 16),
                      ),
                    ),
                    Padding(
                      padding: const EdgeInsets.fromLTRB(30, 20, 0, 10),
                      child: Text(
                        data[orderId]["date"] , style: TextStyle(fontSize: 16),
                      ),
                    ),
                  ],
                ),
                ...
                child: Row(
                  children: <Widget>[
                    Padding(
                      ...
                      child: Text(
                        "Fecha de entrega:", style: TextStyle(fontSize: 16),
                      ),
                    ),
                    Padding(
                      padding: const EdgeInsets.fromLTRB(50, 10, 0, 10),
                      child: Text(
                        data[orderId]["deliveryDate"], style: TextStyle(fontSize: 16),
                      ),
                    ),
                  ],
                ),
              ...
              Flexible(
                  child: ListView.builder(
                      itemCount: orderDetailData == null ? 0 : orderDetailData.length,
                      shrinkWrap:true,
                      itemBuilder: (BuildContext context,j){
                        for(var i = 0; i < laundryServiceMaterialData.length; i++){
                          if(laundryServiceMaterialData[i]['id'] == orderDetailData[j]['laundryServiceMaterialId']){
                            laundryServiceMaterialId = i;
                            for(var k = 0; k < serviceMaterialData.length; k++){
                              if(serviceMaterialData[k]['id'] == laundryServiceMaterialData[laundryServiceMaterialId]['serviceMaterialId']){
                                materialId = serviceMaterialData[k]['materialId'];

                                if(materialId == 1){material = "Algodón";}
                                else if(materialId == 2){material = "Lino";}
                                else if(materialId == 3){material = "Poliester";}
                                else if(materialId == 4){material = "Lana";}
                                else if(materialId == 5){material = "Seda";}
                                else if(materialId == 6){material = "Nylon";  }
                                else{material = "Licra";}
                              }
                            }
                          }
                          ...
                          child: Row(
                            children: <Widget>[
                              Padding(
                                ...
                                child: Text(
                                  material, style: TextStyle(fontSize: 15),
                                ),
                              ),
                              Padding(
                                padding: const EdgeInsets.fromLTRB(120, 20, 0, 20),
                                child: Text(
                                    "s/ " + laundryServiceMaterialData[laundryServiceMaterialId]["price"].toString() + "0", style: TextStyle(fontSize: 15),
                                ),
                              ),
                            ],
                          ),
                ...
                child: Row(
                  children: <Widget>[
                    Padding(
                      ...
                      child: Text(
                        "Total", style: TextStyle(fontSize: 16,fontWeight: FontWeight.bold),
                      ),
                    ),
                    Padding(
                      padding: const EdgeInsets.fromLTRB(140, 30, 0, 40),
                      child: Text(
                        "s/ " + data[orderId]["orderAmount"].toString() + "0", style: TextStyle(fontSize: 16,fontWeight: FontWeight.bold),
                      ),
                    ),
                  ],
                ),
              ...
              MaterialButton(
                ...
                onPressed: (){
                  updateOrderStatus(dropdownValue, widget.user_id, data[orderId]["deliveryAddress"], data[orderId]["orderAmount"], data[orderId]["date"], data[orderId]["deliveryDate"]);
